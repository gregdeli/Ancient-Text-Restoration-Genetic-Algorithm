...
To run the genetic algorithm and reconstruct the damaged inscription:

1. **Prepare the Dataset**: Ensure the dataset `iphi2802.csv` is in the correct directory (`Dataset/`).
2. **Configure Parameters**: Adjust the parameters such as population size, crossover probability, and mutation rate within the script.
3. **Execute the Script**: Run the Python script to begin the optimization process.
4. **Analyze the Results**: Review the output, which includes the reconstructed inscription, average fitness scores, and a plot of the fitness scores across generations.

### Example Output:
```plaintext
Top 10 most similar texts:
Text 0: [Text from the dataset]
Text 1: [Text from the dataset]
...
Fixed inscription: "αλεξανδρε ουδις [word1] [damaged inscription] [word2]"
Average fitness score of the best solution: 0.85
